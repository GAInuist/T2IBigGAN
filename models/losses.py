...
class WGANGP(Loss):
    ...
    def loss_d_additional(self, forward_fn, input_real, input_fake, sent_emb):
        alpha = torch.rand(input_real.data.size(0), 1, 1, 1, device=self.device)
        interpolates = torch.tensor(input_real.data.clone().detach() * alpha + input_fake.data * (1 - alpha),
                                    requires_grad=True)

        disc_interpolates = forward_fn(interpolates, sent_emb)
